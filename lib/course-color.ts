 ...
 *
 * Used where the week on screen is one person's — /courses and the guest demo.
 * A course has no colour of its own in a group, where the colour is whose block
 * it is, so a swatch that survived into the group view would be contradicting
 * the grid two inches below it.
 */
